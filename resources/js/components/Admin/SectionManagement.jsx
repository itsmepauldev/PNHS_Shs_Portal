...
const handleEditSection = async (section) => {
  ...
  const handleAddSection = async () => {
    const adviserOptions = advisers.map(adviser => `<option value="${adviser.id}">${adviser.name}</option>`).join('');
    const currentYear = new Date().getFullYear();
    ...
    const { value: formValues } = await Swal.fire({
      ...
      html:
        '<div class="d-flex flex-column gap-2">' +
        '<input id="swal-section-name" class="swal2-input" placeholder="Section Name" style="margin: 0;" />' +
      ...
      preConfirm: async () => {
        ...
        const duplicate = sections.find(s => s.section_name.toLowerCase() === section_name.toLowerCase() && s.school_year === school_year);
        if (duplicate) {
  ...
  <strong>Current Timeline:</strong> SY {currentTimeline.schoolYear}, {currentTimeline.semester}
        ...
        <div className="row mb-3">
          <div className="col-md-4 mb-2">
            <input type="text" className="form-control" placeholder="Search by section name" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="col-md-4 mb-2">
...
<option value={currentTimeline.semester}>Current: {currentTimeline.semester === '1st' ? '1st Semester' : '2nd Semester'}</option>
  ...
  <button className="btn btn-sm btn-secondary flex-fill" onClick={() => navigate(`/admin/section/${section.id}`)}>Manage</button>
  <button className="btn btn-sm btn-warning flex-fill" onClick={() => handleEditSection(section)}>Edit</button>
  <button className="btn btn-sm btn-danger flex-fill" onClick={() => handleDeleteSection(section.id)}>Delete</button>
</div>
                  </div>
                </div>
              </div>
