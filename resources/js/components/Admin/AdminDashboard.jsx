...
export default function AdminDashboard() {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    html:
      ...
      // '<option value="adviser">Adviser</option>' + // ✅ Added Adviser option
...
const handleEdit = async (user) => {
  ...
  `<option value="adviser" ${user.role === 'adviser' ? 'selected' : ''}>Adviser</option>` +
    ...
    didOpen: () => {
      ...
      updatePasswordField(roleSelect.value); // Set initial visibility
...
<div className="row g-3 mb-4">
  <div className="col-12 col-sm-6 col-lg-3">
    <div className="card text-white bg-primary h-100">
      <div className="card-body">
        ...
        <p className="card-text fs-3">{countByRole('student')}</p>
      </div>
    </div>
  </div>
  <div className="col-12 col-sm-6 col-lg-3">
    <div className="card text-white bg-success h-100">
      <div className="card-body">
        ...
        <p className="card-text fs-3">{countByRole('teacher')}</p>
      </div>
    </div>
  </div>
   <div className="col-12 col-sm-6 col-lg-3">
    <div className="card text-white bg-warning h-100">
      <div className="card-body">
        ...
        <p className="card-text fs-3">{countByRole('adviser')}</p>
      </div>
    </div>
  </div> 
  <div className="col-12 col-sm-6 col-lg-3">
    <div className="card text-white bg-danger h-100">
      <div className="card-body">
        <h5 className="card-title">Document Requests</h5>
  ...
  {currentUsers.length > 0 ? (
    currentUsers.map((user, index) => (
      <tr key={user.id}>
        <td>{indexOfFirstUser + index + 1}</td> {/* ✅ Row Number */}
        <td>{user.name}</td>
        <td>{user.email}</td>
